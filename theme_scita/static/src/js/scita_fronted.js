odoo.define('theme_scita.scita_frontend_js', function(require) {
    ...
    animation.registry.theme_scita_category_slider = animation.Class.extend({
    ...
    animation.registry.second_category_slider = animation.Class.extend({
    ...
    animation.registry.blog_7_custom_snippet = animation.Class.extend({
        selector: ".blog_7_custom",
        start: function() {
            var self = this;
            if (this.editableMode) {
                var $blog_snip = $('#wrapwrap').find('#blog_custom_7_snippet');
                var blog_name = _t("Blog Slider")
            ...
            if (!this.editableMode) {
                var slider_type = self.$target.attr('data-blog-slider-type');
                $.get("/theme_scita/seven_blog_get_dynamic_slider", {
                    'slider-type': self.$target.attr('data-blog-slider-type') || '',
                }).then(function(data) {
                    if (data) {
                        self.$target.empty();
                        self.$target.append(data);
                        $(".blog_7_custom").removeClass('o_hidden');
                        ajax.jsonRpc('/theme_scita/blog_image_effect_config', 'call', {
                            'slider_type': slider_type
                        }).done(function(res) {
    ...
    animation.registry.s_theme_scita_client_slider_snippet = animation.Class.extend({
        ...
        start: function() {
            ...
            if (!this.editableMode) {
                $.get("/theme_scita/get_clients_dynamically_slider", {}).done(function(data) {
                    if (data) {
                        self.$target.empty();
                        self.$target.append(data);
    ...
    animation.registry.second_client_slider_snippet = animation.Class.extend({
        ...
        start: function() {
            ...
            if (!this.editableMode) {
                $.get("/theme_scita/second_get_clients_dynamically_slider", {}).done(function(data) {
                    if (data) {
                        self.$target.empty();
                        self.$target.append(data);
                    }
    ...
    animation.registry.third_client_slider_snippet = animation.Class.extend({
        selector: ".third-client-slider",
        start: function() {
            var self = this;
            if (this.editableMode) {
                var $client_slider = $('#wrapwrap').find('#third_custom_client_slider');
                var client_name = _t("Our Partners")
            ...
            if (!this.editableMode) {
                $.get("/theme_scita/third_get_clients_dynamically_slider", {}).done(function(data) {
                    if (data) {
                        self.$target.empty();
                        self.$target.append(data);
                    }
            ...
            if (this.editableMode) {
                ...
                _.each($team_one, function (single){
                    $(single).empty().append('<div class="block-title" contentEditable="false">\
                        <h3 class="fancy">Our Awesome Team</h3>\
                    </div>');
                });
            }
            if (!this.editableMode) {
    ...
    animation.registry.our_team_varient_5 = animation.Class.extend({
        ...
        start: function() {
            ...
            if (this.editableMode) {
                ...
                _.each($team_one, function (single){
                    $(single).empty().append('<div class="block-title" contentEditable="false">\
                        <h3 class="fancy">Meet Our Team</h3>\
                    </div>');
                });
            }
            if (!this.editableMode) {
