...
class website(models.Model):
    ...
    def get_colors_scss(self):
        ...
        if self.id:
            scss = '/static/src/scss/options/colors/website_' + \
                str(self.id)+'_color_picker.scss'
        module_str = '/'.join((os.path.realpath(__file__)).split('/')[:-2])
        try:
            f = open(module_str + scss, 'r')
            f.close()
            return '/theme_scita'+scss
        except:
            return '/theme_scita/static/src/scss/colors/color_picker.scss'
    ...
    def theme_scita_payment_icons(self):
        ...
        return self.env['payment.icon'].sudo().search([
            ('acquirer_ids.website_published', '=', True)], limit=5)
